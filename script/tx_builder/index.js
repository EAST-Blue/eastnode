...
(async () => {
  ...
  const actionsPacked = Buffer.from(
    borsh.serialize(actionsStruct, params.transaction.actions),
    "hex"
  ).toString("hex");
  // console.log(actionsPacked)

  const txPackedByte = Buffer.from(
    borsh.serialize(transactionStruct, {
      signer: pubKey.toString("hex"),
      receiver:
  ...
  const mainPacked = Buffer.from(
    borsh.serialize(mainStruct, {
      id: txID,
      signature: signature,
  ...
  console.log("mainPacked", mainPacked);
  ...
  const response = await fetch("http://localhost:3000/", {
    body:
      '{"jsonrpc": "2.0", "method": "runtime.Mutate", "params": ["' +
      mainPacked +
      '"]}',
    headers: {
